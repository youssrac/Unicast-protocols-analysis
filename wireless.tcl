...
set opt(ifqlen)       50    ;# max packet in q 
set opt(nn)           40
set opt(connections)  40
set opt(stop)         100
...
for {set i 0} {$i < $opt(nn) } {incr i} {
    $node_($i) set X_ [expr rand()*rand()*100]
    $node_($i) set Y_ [expr rand()*100]
    $node_($i) set Z_ 0
}
...
for {set j 0} {$j < 100} {incr j} {
    for {set i 0} {$i < $opt(nn)} {incr i} {
        ...
        $ns_ at $rng_time "$node_($i) setdest $xx_ $yy_ 50.0"   ;
    }
}
